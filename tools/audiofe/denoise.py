...
def estimate_noise(x, noise_pct=15.0):
    """بصمةُ الضجيج (وسيطُ مقادير أهدأ الإطارات) — مرآةُ `NoiseGate.estimateNoise` (بلا عيّنة 400)."""
    if len(x) < N_FFT * 2:
        return None
    spec, _, _ = stft(x)
    mag = np.abs(spec)
    ...
    return np.median(mag[np.argsort(energy)[:k]], axis=0)


def denoise(x, noise_pct=15.0, over=1.6, floor_db=-14.0, smooth_t=3, smooth_f=3,
            report=False, noise_mag=None):
    """يكتم الضجيج الثابت في [x] ويعيد الصوت (‏و`report=True` معه قياسُ ما فُعل).
    [noise_mag]: بصمةٌ مقدَّرةٌ سلفاً (‏من الصوت كلِّه — كما يفعل المحرك لكلّ مجموعة)."""
    if len(x) < N_FFT * 2:

...
SILENCE_BLOCK_SECONDS = 30


def local_speech_floors(x, floor_fn):
    """مرآةُ `LongAudioTranscriber.localSpeechFloors` (‏D-816): عتبةٌ لكلّ إطارٍ من كتلتِه (30ث)،
    والكتلةُ الأخيرةُ الأقصرُ من ثانيةٍ تُقاس على آخر 30ث. ما دون 30ث ⇒ العتبةُ الكليّةُ نفسُها."""
    frame = SR // 50
    n = len(x) // frame
    out = np.zeros(n)
    block = SILENCE_BLOCK_SECONDS * SR
    for b0 in range(0, len(x), block):
        end = min(b0 + block, len(x))
        blk = x[max(0, end - block):end] if end - b0 < SR else x[b0:end]
        out[b0 // frame:min(n, (b0 + block) // frame)] = floor_fn(blk, long_rec=len(x) > block)
    return out


def al_speech_floor_block(x, long_rec=False):
    """مرآةُ فرع v1 في `localSpeechFloors` (‏D-817): في تسجيلٍ أطولَ من كتلةٍ حدُّ الحصر الأدنى
    ‏2 × quiet_floor(الكتلة) محصوراً في [0.001, 0.004] بدل 0.004، والسقفُ 0.02 يُقدَّم."""
    if not long_rec:
        return al_speech_floor(x)
    import snr_probe as _sp
    fr = 320
    n = len(x) // fr
    if n == 0:
        return 0.01
    e = np.sqrt((x[: n * fr].reshape(n, fr).astype(np.float64) ** 2).mean(axis=1))
    lo = min(max(2.0 * _sp.quiet_floor(e), 0.001), 0.004)
    return float(min(max(np.sort(e)[min(max(int(n * 0.85), 0), n - 1)] * 0.1, lo), 0.02))


def split_at_silences(x, threshold, min_silence_ms=MIN_SILENCE_MS, min_utt_ms=MIN_UTTERANCE_MS):
    """[threshold]: رقمٌ للكلّ أو مصفوفةٌ لكلّ إطار (‏[local_speech_floors])."""
    frame = SR // 50                      # 20 م.ث
    n = len(x) // frame
    if n == 0:
        return [(0, len(x))]
    e = np.sqrt((x[: n * frame].reshape(n, frame).astype(np.float64) ** 2).mean(axis=1))
    quiet = e < (threshold[:n] if isinstance(threshold, np.ndarray) else threshold)
...
class Transcriber:
    ...
    def __init__(self, model_path, device="cpu", frontend="old", threads=4, gate=False,
                 ...
                 vad_model=None, vad_threshold=0.5):
        # 🗣️ M1-1 (‏2026-09-18): Silero VAD المدمج في whisper.cpp — يُسقط غيرَ الكلام **داخل** النافذة قبل الفكّ.
        # مسارُ cli وحدَه؛ `None` = المسارُ المشحون بلا فرق.
        self.vad_model, self.vad_threshold = vad_model, vad_threshold
    ...
    def _raw(self, audio, bias_ids=None, bias=0.0):
        ...
        if self.backend == "cli":
            ...
            if self.vad_model:
                cmd += ["--vad", "-vm", self.vad_model, "-vt", f"{self.vad_threshold:g}"]
            ...
            # ⚠️ مع VAD قد تكون النافذةُ كلُّها غيرَ كلامٍ فعلاً (ضجيجٌ بلا تلاوة) — فالفراغُ هنا حكمٌ لا عطب.
            if not txt and len(audio) > SR and not self.vad_model:
    ...
    def transcribe(self, raw_audio, bias_ids=None, bias=0.0, ref_text=None, lam=0.0, k=5):
        ...
        if self.frontend == "v2post":
            return self._transcribe_v2post(raw_audio, bias_ids, bias, ref_text, lam, k)
        if self.frontend == "engine-old":
            return self._transcribe_engine_old(raw_audio, bias_ids, bias, ref_text, lam, k)
        if self.gate:
        ...
        if self.frontend == "new":
            ...
            floor = local_speech_floors(audio, al_speech_floor_block)   # D-816/D-817
    ...
    def _transcribe_v2post(self, raw_audio, bias_ids, bias, ref_text, lam, k):
        """🎚️ **مرآةُ ترتيب المحرك مع v2 المُصلَحة** (‏خطة 2026-09-23 §5-د · D-259/D-493):
        الكسبُ يُقدَّر على الخام (‏أرضيةٌ كاملة) ⇒ التقطيعُ على المسوّى ⇒ بصمةُ الضجيج من **الخام** ⇒
        لكلّ مجموعةٍ: بوّابةٌ على الخام **ثمّ** الكسبُ المقدَّر سلفاً. (‏`LongAudioTranscriber` عند `AudioLevelV2.enabled`.)"""
        import frontend as fe
        import denoise as _dn
        gain_db = fe.plan_v2(raw_audio)
        audio = fe.apply_v2(raw_audio, gain_db)
        floor = local_speech_floors(audio, lambda b, long_rec=False: fe.speech_floor_v2(b))   # D-816
        utts = split_at_silences(audio, floor)
        if len(utts) > 1:
            groups = group_utterances(utts, self.group_cap, MAX_GAP_SECONDS * SR)
        else:
            groups = [(0, utts[0][1] if utts else len(audio))]
        prof = _dn.estimate_noise(raw_audio) if self.gate else None
        window = WINDOW_SECONDS * SR
        parts = []
        for a, b in groups:
            seg = raw_audio[a:min(b, len(raw_audio))]
            if self.gate:
                seg = _dn.denoise(seg, noise_mag=prof)
            seg = fe.apply_v2(seg, gain_db)
            parts.append(self._pick(seg, bias_ids, bias, ref_text, lam, k) if len(seg) <= window + SR
                         else self._windowed(seg, bias_ids, bias, ref_text, lam, k))
        return " ".join(p for p in parts if p), len(groups)

    def _transcribe_engine_old(self, raw_audio, bias_ids, bias, ref_text, lam, k):
        """مرآةُ ترتيب المحرك المشحون (v1): تسويةٌ ⇒ تقطيع ⇒ بصمةٌ من المسوّى ⇒ بوّابةٌ لكلّ مجموعة."""
        import denoise as _dn
        audio = al_normalize(raw_audio)
        utts = split_at_silences(audio, al_speech_floor(audio))
        if len(utts) > 1:
            groups = group_utterances(utts, self.group_cap, MAX_GAP_SECONDS * SR)
        else:
            groups = [(0, utts[0][1] if utts else len(audio))]
        prof = _dn.estimate_noise(audio) if self.gate else None
        window = WINDOW_SECONDS * SR
        parts = []
        for a, b in groups:
            seg = audio[a:min(b, len(audio))]
...
def main():
    ...
    ap.add_argument("--frontend", default="old", choices=["old", "new", "engine-old", "v2post"],
                    help="الواجهة الأمامية: old = المشحونة · new = frontend.py (‏M0-5) · "
                         "engine-old = ترتيبُ المحرك المشحون (بوّابةٌ لكلّ مجموعة) · v2post = v2 مقدَّرةٌ قبل البوّابة مطبَّقةٌ بعدها")
    ...
    ap.add_argument("--vad", default="", help="مسارُ نموذج Silero ggml — يفعّل VAD داخل whisper-cli (‏M1-1)")
    ap.add_argument("--vad-threshold", type=float, default=0.5, help="عتبةُ VAD (‏0..1)")
    ...
    if args.vad:
        if args.backend != "cli":
            raise SystemExit("⛔ --vad لمسار cli وحدَه")
        suffix += f"_vad{args.vad_threshold:g}"
    ...
    tr = Transcriber(model_path, frontend=args.frontend, threads=args.threads, gate=args.gate,
                     ...
                     floor_rule=args.floor_rule, vad_model=args.vad or None, vad_threshold=args.vad_threshold)
    if args.cuts_json:
    ...
    meta = {"model": args.model, "modelPath": str(model_path), "set": tag,
            ...
            "lang": args.lang,
            "vad": (f"silero {os.path.basename(args.vad)} t={args.vad_threshold:g}" if args.vad else None),

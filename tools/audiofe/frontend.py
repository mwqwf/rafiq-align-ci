...
def plan_v2(x):
    """🎚️ **تقديرُ التسوية قبل البوّابة** (‏D-259 · خطة 2026-09-23 §5-د) — مرآةُ `AudioLevelV2.plan`.
    يعيد الكسبَ بالديسيبل (‏0 = بلا مساس) محسوباً على الصوت **الخام** وأرضيتُه كاملة."""
    if len(x) == 0:
        return 0.0
    _, info = normalize_v2(x, report=True)
    return float(info.get("gainDb", 0.0))


def apply_v2(x, gain_db):
    """تطبيقُ كسبٍ مقدَّرٍ سلفاً (‏بعد البوّابة) + المحدِّد الناعم — مرآةُ `AudioLevelV2.applyPlan`."""
    if gain_db <= 0.01 or len(x) == 0:
        return x
    return soft_limit(x * (10 ** (gain_db / 20.0)))
...
def speech_floor_prop(x, margin_db=4.0, cap=0.9):
    """🔪 **عتبةُ السكوت بسقفٍ يسمح بالسكوت في الضجيج** — مرشَّحُ D-345 بعد تصحيحه (‏D-346).

    ⛔ **العطبُ الحقيقيّ — والسقفُ هو العلّةُ لا الهامش:** `speech_floor_v2` تحصر العتبةَ
    بـ`speech_level × 0.5`. وفي الضجيج تكون **أرضيةُ الضجيج نفسُها أعلى من نصف مستوى الكلام**،
    فتنزل العتبةُ تحت الضجيج ⇒ **لا إطارَ يُعدّ صامتاً أصلاً** ⇒ `splitAtSilences` تُعيد **نطقاً
    واحداً** ⇒ تُبنى النوافذُ عمياءَ 25ث ويُتجاوز مسارُ التجميع والسقف كلُّه.

    والمقيسُ (‏20 بنداً · البندُ نفسُه نظيفاً ومضجَّجاً 5 د.ب · مسحُ الهامش والسقف):

    | السقف | نظيف | مضجَّج | مضجَّجٌ بنطقٍ واحد (انهيار) |
    |---|---:|---:|---:|
    | **0.5 (المشحون)** | 6 | **1** | **20/20** |
    | 0.7 | 6 | 3 | 6/20 |
    | **0.9** | 6 | **8** | **0/20** |

    ⭐ **والنظيفُ 6 في كلّ خليّةٍ من المسح** (هامشٌ 2…8 × سقفٌ 0.5…1.2) ⇒ **صفرُ انحدارٍ حيث لا
    ضجيج**، وهو شرطُ القبول بعد درس D-264 (‏20.9 نقطة). والهامشُ لا يكاد يؤثّر متى رُفع السقف.
    """
    nf = noise_floor(x)
    sp = speech_level(x)
    floor = nf * (10 ** (margin_db / 20.0))
    return float(min(max(floor, 1e-4), max(sp * cap, 1e-4)))

...
def main():
    ...
    # ⛔ خضرةٌ كاذبةٌ وقعت فعلاً (‏2026-09-12): على `g3r:noisy` طبعت «صفرُ انحراف» و**صفرُ
    # كلماتٍ مقارَنة** — لأنَّ بنودَ الحشو ليست في `sample.json` فخرجت `items` فارغةً. فلا خضرةَ بلا شهادة.
    if words == 0:
        sys.exit(f"⛔ صفرُ كلماتٍ مقارَنة على {a.set}: معرّفاتُ هذه المجموعة ليست في `sample.json` "
                 f"(بنودٌ بأحكامٍ من المحرك: {len(with_judge)}) ⇒ لا حكمَ لا سلباً ولا إيجاباً. "
                 "‏g3r يُصدّق بخطّة الحشو (`inject_plan_riwaya.json`) لا بالعيّنة.")

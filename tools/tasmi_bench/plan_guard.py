# -*- coding: utf-8 -*-
"""🧷 **حارسُ الحقيقة الأرضيّة** — يفحص **ثلاثةَ أشكالٍ** من العيّنات قبل أن يُحكم بها،
ويميّزها **من شكل البند نفسِه** لا من اسم الملفّ:

| الشكل | ما يقيسه | أثقلُ شرطٍ فيه |
|---|---|---|
| **خطّةُ حقنٍ** (`inject_plan*.json`) | الكشف | الخطأُ المصنوعُ **موجودٌ فعلاً** في الصوت |
| **عيّنةٌ نظيفة** (`sample.json` · G1) | الاتّهام الكاذب | `refText` **نصُّ المصحف**، والصوتُ **صوتُ هذه الآية** |
| **تلاوةٌ طويلة** (`long_plan.json` · g4) | مسارُ الطويل | النصُّ **سلسلةُ آيات المدى بترتيبها** |

⛔⛔ **وصفرُ بنودٍ ليس سلامةً** (‏D-442): ملفٌّ بلا بنودٍ **يُردّ برمزٍ** ولا يُزكّى — وهو العطبُ
المكتوبُ في دفتر الأعطاب (`judge_parity` طبعت «✅ صفرُ انحراف» على صفر كلمات) **ووقع في هذا
الحارس نفسِه** حتى قِيس بتجربةٍ على مُدخَلٍ فارغ.

...
def score_g3r(set_name, arms=None):
    ...
    lo, hi, p = _boot_diff(pairs)
    # ⛔ **نصفُ الحكم كان بلا مجال** (‏2026-09-13): الحكمُ زوجٌ — «ارتفعت الدقّةُ ولم يرتفع الاتّهام» —
    # والاتّهامُ وحدَه كان له `[95٪]`، أمّا **الكشفُ فيُطبع فرقاً عارياً** (‏«‎−1.3») بخانتَين فارغتَين.
    # ⇒ لا يُعرف أهبوطُ الكشف حقيقةٌ أم ضجيجُ عيّنة، **فيُقرأ الرقمُ على هوى القارئ**: مَن أراد
    # القبولَ سمّاه ضجيجاً، ومَن أراد الردَّ سمّاه انحداراً — وكلاهما بلا دليل. والبندُ هنا بندٌ
    # (‏كُشف الخطأُ أو لم يُكشف) فبوتستراپُه هو `_boot_diff` عينُه بمقامٍ واحدٍ للبند.
    det_pairs = [(pa[i][2], 1, pb[i][2], 1) for i in pa if i in pb]
    d_lo, d_hi, d_p = _boot_diff(det_pairs)
    by = {}
    ...
    return {"set": set_name, "n": n, "detect": (da, db), "fa": (fa_a, fa_b), "fa_diff": fa_b - fa_a, "ci": (lo, hi),
            "p_fa_up": p, "det_diff": db - da, "det_ci": (d_lo, d_hi), "p_det_up": d_p, "by_riwaya": by}
...
def table():
    ...
    for s in G3R_SETS:
        ...
        print(f"| {s} · كشف ⚠️ | {r['n']} | {r['detect'][0]*100:.1f}٪ | **{r['detect'][1]*100:.1f}٪** | "
              f"{r['det_diff']*100:+.1f} [{r['det_ci'][0]*100:+.1f}..{r['det_ci'][1]*100:+.1f}] | {r['p_det_up']*100:.0f}٪ | |")
    ...
    # ⛔⛔ **لافتةٌ تمنع قراءةً وقعت فعلاً** (‏2026-09-13): قرأت مناوبةٌ صفَّ «كشف» عارياً فحكمت
    # «الكشفُ ينزل ⇒ لا يُربط ورشٌ وقالون بالأكبر»، وقرأت أخرى البياناتِ عينَها بمقياسٍ يرى
    # الزوائدَ فوجدت الكشفَ **يرتفع** (‏84.3 ⇒ 87.5٪) ⇒ **حكمان متعاكسان من شوطٍ واحد**.
    # والسببُ مكتوبٌ من قبلُ في D-351: مسطرةُ هذا الصفّ **لا ترى `INSERT`**، فتقرأ تحسّناً
    # في سماع الدخيلة هبوطاً في الكشف. ⇒ الصفُّ يُعلَّم، ولا يُحكم به وحدَه.
    print(f"\n⚠️ **وصفُّ «كشف» مسطرتُه ناقصةٌ** (‏D-351): لا ترى `INSERT` ⇒ نموذجٌ يسمع الكلمةَ "
          "الدخيلةَ أكثرَ قد يظهر فيه **أقلَّ كشفاً**. فهبوطُه **ليس حكماً** حتى يُفصَّل بالصنف "
          "(`detect_anatomy.py`)، وارتفاعُ `INSERT` وحدَه قد يقلب الإشارة.")
    return rows


# ---- 🧪 اختبارٌ ذاتيٌّ للحاكم (أُضيف 2026-09-14 · مناوبةُ :13) ----
# ⛔ **لِمَ:** هذا الملفُّ **يقرّر شحنَ نموذجٍ من عدمه** (الحكمُ الزوجيّ: دقّةٌ ترتفع واتّهامٌ لا
# يرتفع)، وكان **بلا اختبارٍ ذاتيٍّ واحد** — بينما القاعدةُ في هذا المستودع: **الحارسُ يُختبر
# قبل أن يُستعمل**. والحالاتُ أدناه **مقيسةٌ لا مفترَضة**: جُرّبت على الحاكم نفسِه قبل كتابتها.
REF_T = "الحمد لله رب العالمين الرحمن الرحيم مالك يوم الدين"


def _item(word_index):
    return [{"id": "t1", "refText": REF_T, "wordIndex": word_index, "op": "SUBSTITUTE", "riwaya": "hafs"}]
...
def selftest():
    ...
    # ① مجالُ الفرق: كسبٌ يقينيّ · خسارةٌ يقينيّة · ذراعان متطابقتان · لا أزواج.
    gain = [(5, 10, 9, 10)] * 12
    lo, hi, p = _boot_diff(gain)
    ok("كسبٌ يقينيٌّ ⇒ المجالُ كلُّه فوق الصفر", (lo > 0, hi > 0, p == 1.0), (True, True, True))
    lo, hi, p = _boot_diff([(9, 10, 5, 10)] * 12)
    ok("خسارةٌ يقينيّةٌ ⇒ المجالُ كلُّه دون الصفر", (lo < 0, hi < 0, p == 0.0), (True, True, True))
    # ⭐ **ذراعان متطابقتان ⇒ مجالٌ صفريُّ العرض** — ودرسُ D-385: هذا **سؤالٌ لا جواب**،
    #    يُسأل عنه النصُّ (أهما نسختان؟) ولا يُقرأ «لا فرقَ مقيساً».
    ok("ذراعان متطابقتان ⇒ [0,0]", _boot_diff([(7, 10, 7, 10)] * 12), (0.0, 0.0, 0.0))
    ok("ولا أزواجَ ⇒ أصفارٌ بلا انفجار", _boot_diff([]), (0.0, 0.0, 0.0))
    # ② والبذرةُ تجعل الحكمَ قابلاً للإعادة — نزعُها يجعل الرقمَ يرقص بين قراءتَين.
    ok("بذرةٌ واحدةٌ ⇒ نتيجةٌ واحدة", _boot_diff(gain) == _boot_diff(gain), True)

    # ③ حاكمُ الحقن: كشفٌ · واتّهامٌ بعيدٌ · ونظيفةٌ · وفرضيّةٌ غائبة (‏أرقامٌ مقيسةٌ أعلاه).
    clean = {"t1": {"text": REF_T}}
    err6 = {"t1": {"text": "الحمد لله رب العالمين الرحمن الرحيم ملك يوم الدين"}}
    det, fa, n, per = judge_arm(_item(6), clean)
    ok("تلاوةٌ مطابقة ⇒ لا كشفَ ولا اتّهام", (det, fa, n, per["t1"]), (0.0, 0.0, 1, (0, 6, 0)))
    det, fa, n, per = judge_arm(_item(6), err6)
    ok("خطأٌ في موضع الحقن ⇒ كشفٌ بلا اتّهام", (det, fa, per["t1"]), (1.0, 0.0, (0, 6, 1)))
    det, fa, n, per = judge_arm(_item(0), err6)
    ok("الخطأُ نفسُه بعيداً عن الحقن ⇒ اتّهامٌ بلا كشف", (det, per["t1"]), (0.0, (1, 7, 0)))
    ok("وبند بلا فرضيّةٍ لا يُحتسب أصلاً", judge_arm(_item(6), {})[2], 0)

    # ⚠️ **وحدٌّ قِيس ويُقال:** حارسُ الانهيار (0.60) يَعُدّ `MISSED`/`SUBSTITUTED` وحدَها،
    #    و**التلاوةُ المبتورةُ أو المنحرفةُ جملةً تُعطي `UNCERTAIN`** (قِيس: ثلاثُ كلماتٍ من
    #    تسعٍ ⇒ صفرُ `CONF`) ⇒ **لا يشتعل الحارسُ عليها**. فمجالُه: انحرافٌ **واثقٌ** واسع،
    #    لا مجرّدُ اختلافٍ كبير. ⛔ ولا يُقال «لا يشتعل أبداً» — يشتعل في الأشواط الحقيقيّة.
...
def main():
    ...
    ap.add_argument("--selftest", action="store_true", help="يختبر الحاكمَ على حالاتٍ مقيسةٍ بلا محاكٍ")
    a = ap.parse_args()
    if a.selftest:

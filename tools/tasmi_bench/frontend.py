...
def speech_floor_prop(x, alpha=0.24, lo_db=2.0, hi_db=6.0):
    """🔪 **عتبةُ السكوت بهامشٍ نسبيٍّ لا ثابت** — مرشَّحُ D-345، مرآةٌ لما يُقترح للمحرك.

    ⛔ **العطب:** `speech_floor_v2` هامشُها **ثابتٌ ‎+6 د.ب** فوق الأرضية. وعند نسبةِ إشارةٍ 5 د.ب
    يكون الكلامُ نفسُه نحوَ 5 د.ب فوق الأرضية ⇒ **العتبةُ أعلى من كثيرٍ من الكلام**، فتُحسب 54٪ من
    الإطارات صمتاً ويُقطَّع النطقُ في أثنائه: **14 نطقاً بدل 6**، و**53٪ من القطوع داخلَ كلامٍ مسموع**.

    **والقاعدةُ هنا:** الهامشُ نسبةٌ من المدى الديناميّ محصورةٌ في [lo_db, hi_db] ⇒ يضيق حين يضيق
    المدى ويبقى كما هو حين يتّسع. والمقيسُ (‏25 بنداً · البندُ نفسُه في أربع طبقات):

    | المجموعة | الهامش | نطقاتٌ وسيطاً (الصوابُ 6) | قطوعٌ داخلَ كلام |
    |---|---:|---:|---:|
    | نظيف | 6.0 ⇒ **6.0** | 6 ⇒ **6** | 0٪ ⇒ **0٪** |
    | 20 د.ب | 6.0 ⇒ **6.0** | 8 ⇒ **8** | 4٪ ⇒ **4٪** |
    | 10 د.ب | 6.0 ⇒ **4.2** | 10 ⇒ **9** | 22٪ ⇒ **7٪** |
    | **5 د.ب** | 6.0 ⇒ **3.2** | **14 ⇒ 6** | **53٪ ⇒ 8٪** |

    ⭐ **والأهمُّ أنّها مطابقةٌ حرفياً في النظيف والخفيف** (الهامشُ يبلغ السقفَ فيعود ‎+6) ⇒ لا انحدارَ
    حيث لا ضجيج. وهذا شرطُ القبول بعد درس D-264 (تغييرُ ترتيب الكتم والتقطيع كلّف 20.9 نقطة).
    """
    import math
    nf = noise_floor(x)
    sp = speech_level(x)
    if nf <= 0 or sp <= nf:
        return speech_floor_v2(x)
    mg = min(hi_db, max(lo_db, alpha * 20.0 * math.log10(sp / nf)))
    floor = nf * (10 ** (mg / 20.0))
    return float(min(max(floor, 1e-4), max(sp * 0.5, 1e-4)))

...
def selftest():
    ...
    # 🛑⛔⛔ **وضابطُ العلَم — عطبٌ كان قائماً وكشفه أوّلُ قياسٍ بالباب** (‏D-537):
    #     `score` كان يُخرج `collapsed` بمقايسة الهويّة **في سطر الإرجاع**، وهذه المرحلةُ
    #     تُعيد قائمةً جديدةً متى أطلقت ⇒ **فالبابُ وحدَه كان يرفع علَمَ الانهيار كذباً**،
    #     وصفُّ «بنودٌ أطلق فيها حارسُ الانهيار» في كلّ جدولٍ يُقاس بالباب **كذبٌ لا يصرخ**.
    #     ⭐ والحالةُ هنا بعينها: إبدالٌ واحدٌ من ستٍّ (‏0.17 ⇒ **دون** عتبة 0.60).
    ok("🛑 والبابُ يُغفر ولا يرفع علَمَ الانهيار", sB["collapsed"], False)
    ok("🛑 والعلَمُ نفسُه باقٍ عاملاً بالباب (‏أغلبُه لا-كلمات ⇒ ينهار)",
       scorer.score(ref, "الحمد زقزق سسنسا ططط ثثث ذذذ", c2)["collapsed"], True)

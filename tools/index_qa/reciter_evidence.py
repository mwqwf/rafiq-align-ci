...
# ⭐ **نافذةُ الشاهد الصالح — مقيسةٌ لا مفترَضة** (2026-09-09):
#    س106 (4 آية · تفريقٌ 5.9%) ⇒ مطابقةٌ **0.36** · س96 (19 · 4.2%) ⇒ **0.58** ·
#    س107 (7 · 4.0%) ⇒ **0.59** — ثلاثتُها **دون حدّ الحارس الثالث (0.60)**
#    فتُردّ. وس46 (35 آية) وس30 (60) وس89 (30) مطابقتُها **0.60–0.73** فتمرّ.
#    ⇒ whisper يحتاج سياقاً: القِصَرُ يقتل التفريغَ قبل أن ينفع التفريقُ.
MIN_AY, MAX_AY = 25, 60     # حدّا آياتِ الشاهد: سياقٌ كافٍ · ومسبارٌ رخيص
...
def _diff_rows(riwaya: str) -> list:
    """`(‏فرقٌ, سورةٌ, آياتُها, كلماتٌ تفترق, جملةُ الكلمات)` مرتّبةً بالفرق نازلاً."""
    from normalizer_audit import words_folded as W
...
def best_witnesses(riwaya: str, n: int = 2) -> list:
    """⭐ **الشاهدُ أطولُ ما يفرّق لا أقواه تفريقاً** — يُقاس ولا يُخمَّن.

    يردّ سورَ الشاهد لهذه الرواية: فرقٌ > 0 (‏وإلا `conflict` دائم) وطولٌ في
    `MIN_AY..MAX_AY` (‏وإلا انهار التفريغُ تحت حدّ الحارس الثالث). وهذه هي
    الدالّةُ التي يُبنى عليها افتراضُ `probe_new_reciter --witness auto`،
    فلا يُورَث رقمٌ مكتوبٌ في ترويسة.
    """
    usable = [s for d, s, ay, *_ in _diff_rows(riwaya)
              if d > 0 and MIN_AY <= ay <= MAX_AY]
    if len(usable) < n:
        raise SystemExit(f"⛔ {riwaya}: شهودٌ صالحون {len(usable)} والمطلوب {n}")
    return usable[:n]
...
def witness_surahs(riwaya: str, top: int = 12) -> int:
    ...
    # ⭐ **شرطان لا شرط** (‏قياس 2026-09-09): الحارسُ الثالث يشترط مطابقةً
    #    ≥60% لنصّ الرواية، والرابعُ يشترط ألّا يتعادل الشاهدان. وهما
    #    **متعاكسان في السور**: أقوى ما يفرّق أقصرُ ما يُفرَّغ.
    usable = [(d, s, ay) for d, s, ay, *_ in rows if d > 0 and MIN_AY <= ay <= MAX_AY]
    trap = [(d, s, ay) for d, s, ay, *_ in rows if d > 0 and ay < MIN_AY][:3]
    if trap:
        print(f"\n⛔ و**أقوى ما يفرّق فخٌّ لا شاهد** — دونَ {MIN_AY} آيةً "
              "لا يجد whisper سياقاً فتهبط المطابقةُ تحت حدّ الحارس الثالث "
              "(‏قِيس: " +
              " · ".join(f"س{s} ({d:.1%}) ⇒ مطابقةٌ منهارة" for d, s, _ in trap) +
              ").")
    if not usable:
        print(f"\n⛔ لا شاهدَ يستوفي الشرطين في {riwaya} — تُرفع للمشرف.")
        return 1
    print(f"\n⇒ الشاهدان المقترحان لـ{riwaya}: "
          f"**{usable[0][1]} و{usable[1][1]}** "
          f"(‏{usable[0][2]} و{usable[1][2]} آيةً · فرقٌ {usable[0][0]:.1%} "
          f"و{usable[1][0]:.1%}) — **أطولُ ما يفرّق، لا أقواه تفريقاً.**")
    print(f"   وبقيّةُ الصالح (‏{MIN_AY}–{MAX_AY} آية · فرقٌ > 0): " +
          " · ".join(f"س{s} ({ay} آية · {d:.1%})" for d, s, ay in usable[2:6]))
    return 0

...
3. ⭐ **شاهدا الهويّة**: عيّنتان سمعيّتان تُطابقان **نصَّ الرواية المعلَنة**،
   و⛔ **يُرفض من طابق حفصاً أكثر من روايته** — وهذا كاشفُ «النسبةِ الخاطئة»:
   مصحفٌ بحفصٍ مُعلَنٌ ورشاً يمرّ كلَّ فحصٍ آخر ولا يمرّ هذا.
   ⛔⛔ **وله شرطان لا يعمل بدونهما، كلاهما مقيسٌ 2026-09-07:**
   · **المسطرة:** الدرجتان تُبنيان بالمطبِّع **المطويّ** (`normalizer: "folded"`)،
     وإلا انقلب الكاشفُ في الاتجاه الخطر — **42.5% من 400 صفٍّ حقيقيٍّ لتلاوة
     حفصٍ فازت فيها ورشٌ** بالمطبِّع الخام، و**0.0%** بالمطويّ. والحارسُ يردّ
     الآن كلَّ شاهدٍ لم تُسمَّ مسطرتُه.
   · **الشاهدُ يجب أن يفرّق:** كانت الترويسةُ تسمّي **1 و112**، و**112 نصُّها
     واحدٌ في الروايتين** (‏سبعُ سورٍ كذلك) فدرجتاها متساويتان دائماً ⇒ `conflict`
     لكلّ مرشَّح ⇒ مسبارٌ ثالثٌ مهدورٌ على كلٍّ من الخمسين. **والمقاسُ بديلاً:
     89 و104** (‏فرقٌ 14.6% و12.1% في ورش · 13.9% و12.1% في قالون، وكلتاهما قصيرة).
     يُعاد القياسُ بـ`reciter_evidence.py --witness-surahs <رواية>`.
...
def check_evidence(ev, riwaya):
    ...
    for w in wit:
        ...
        # ⛔⛔ **المسطرةُ تُشترط في الملفّ، فالوصيّةُ لا تحرس** (‏قِيس 2026-09-07 على
        #    **1000 صفٍّ حقيقيٍّ** من تقارير `audio_qa`): بالمطبِّع الخام `run.words`
        #    تفوز **ورشٌ على حفصٍ في 42.5%** من صفوف تلاواتِ حفص، وبالمطويّ **0.0%**
        #    (400/400). والعلّةُ أن التفريغ **بلا ألفِ وصلٍ قطعاً** (صفرُ `ٱ` في 1000
        #    صفّ) ومرجعُ حفصٍ يحملها في 16.4% من كلماته وورشٌ صفراً ⇒ الخامُ يعاقب
        #    مرجعَ حفصٍ وحدَه، فيعمى الكاشفُ **في الاتجاه الخطر بعينه**: مصحفُ حفصٍ
        #    يُعلَن ورشاً فيمرّ. ⇒ درجةٌ لم تُبنَ بالمطويّ **لا تُقرأ أصلاً**.
        #    البرهانُ: `reciter_evidence.py --prove` · والبناءُ: `--assemble`.
        if w.get("normalizer") != "folded":
            bad.append(f"3: شاهدُ س{w.get('surah')} درجتُه بمطبِّع "
                       f"«{w.get('normalizer') or 'غيرِ مسمّى'}» لا «folded» — "
                       "تُعاد بـ`reciter_evidence.py --assemble`")
            continue
    ...
    # ⛔⛔ **الحارسُ الخامس أُعيدت معايرتُه بقياس (‏إذنُ المالك النصّيّ
    #    2026-09-09: «أعطيك القرار فيه مهما كان مدمّراً، المهمّ ألّا نُتّهم
    #    بسببه بتحريف القرآن»).**
    #
    #    **العلّةُ في القديم:** الحدُّ المطلق 0.25 يردّ تلاواتٍ **نشحنها اليوم**.
    #    قِيس بمقياس الحارس نفسِه (‏مدّةُ الملفّ ÷ حروفٌ بلا حركة) على قرّاءٍ
    #    **مشهودين منشورين**: `yahya` س98 = **0.198** وس6 (‏الأنعام، 165 آية،
    #    12,571 حرفاً، 45 دقيقة) = **0.216** · `kh_mohammadi` س6 = **0.231**.
    #    ⇒ الحدُّ لم يكن يفصل السليمَ من المبتور، بل **يقطع ذيلَ الحدر**، وحبس
    #    سبعةَ مصاحفَ كاملةٍ شواهدُها نظيفة.
    #
    #    **والخطرُ الحقيقيّ الذي يجب أن يُحرَس هو التحريف**: صوتٌ أقصرُ من نصّه
    #    ⇒ آياتٌ تُسنَد إلى صوتٍ لا يحويها ⇒ **يسمع الحافظُ سورةً ناقصة**.
    #    ⛔ **والحكمُ الصوتيُّ وحده لا يكفي حارساً**: `akri_qalun/24` صوتُه 39%
    #    من طوله ومرّ بحكمٍ 3.78% لأنّ العيّنة العنقودية لم تقع على السورة.
    #
    #    ⇒ **حارسان بدل واحد**، والثاني أقوى على التحريف من القديم كلِّه:
    #      (أ) **حدٌّ مطلقٌ 0.15** — دون أدنى ما قِيس عند مشهودٍ (0.198) بهامش،
    #          ويبقى رادّاً للبتر الجسيم (‏`anoun_warsh` 0.11 · `mudhaffar` 0.10).
    #      (ب) **حدٌّ نسبيٌّ داخل القارئ نفسِه: 60% من وسيطه.** والبترُ شذوذٌ
    #          **داخل** التلاوة لا بين التلاوات — فسورةٌ نصفُ إيقاعِ صاحبها
    #          مبتورةٌ ولو كان القارئُ حادراً، وسورةٌ بإيقاعه سليمةٌ ولو كان
    #          أسرعَ الناس. **وهذا يمسك ما أفلت من الحكم الصوتيّ.**
    vals = [v.get("durationPerLetter") for v in files.values()
            if v.get("durationPerLetter") is not None]
    ...
    for s, v in files.items():
        ...
        if not (0.15 <= r <= 4.0):
            bad.append(f"5: س{s} مدّةٌ لكل حرفٍ خارج النطاق ({r:.2f}) — "
                       f"الحدّ 0.15–4.0")
            break
        if med and r < 0.60 * med:
            bad.append(f"5: س{s} إيقاعُها {r:.2f} دون 60% من وسيط القارئ "
                       f"({med:.2f}) — شذوذٌ داخل التلاوة نفسِها، وهو علامةُ بتر")
            break
...
def _self_test() -> None:
    ...
    # ⛔⛔ ودرجةٌ بمسطرةٍ أخرى لا تُقرأ ولو كانت في صالح القبول أو الردّ
    for nz in (None, "run.words"):
        j = base()
        if nz is None:
            j["witnesses"][0].pop("normalizer")
        else:
            j["witnesses"][0]["normalizer"] = nz
        assert any("folded" in x for x in check_evidence(j, "warsh")), \
            f"⛔ قُبل شاهدٌ بمطبِّع {nz!r} — والكاشفُ يعمى به (42.5% انقلاباً مقيساً)"
    print("  ✅ الحُرّاس 1–6: كلٌّ يردّ حالتَه السالبة، والسليمُ يمرّ")
    print("  ✅ ويُردّ شاهدٌ لم تُبنَ درجتُه بالمطبِّع المطويّ — المسطرةُ مشروطةٌ في الملفّ")

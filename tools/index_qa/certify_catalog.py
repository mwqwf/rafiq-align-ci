...
DECLARED_DROP_FATALS = (
    ...
    # ⛔⛔ **أُضيف 2026-09-20 بقياسٍ من حالة `asim`** (سورةُ ص، الناشرُ يردّ
    #    404 عليها وحدَها، والغيابُ معلَنٌ للقارئ برسالةٍ ورمز):
    #    الفهرسُ سليمٌ (`qa.fatal=None` · تغطية 0.9856 · `op=drop_surah:38`)
    #    وأحكامُه الحديثةُ الأربعةُ بلا فاتل — ومع ذلك سقطت الشهادةُ لأنّ
    #    أحكاماً **متقادمةً** على البصمة نفسِها تحمل هذا النصّ.
    #    ⭐⭐ وهو **أثرُ الإسقاط المعلَن لزوماً**: سورةٌ حُذفت بإعلانٍ ⇒ بصمةُ
    #    صوتها غائبةٌ حتماً، فعدُّها «سورٌ بلا برهان تنزيل» وصفٌ صحيحٌ لحالةٍ
    #    معذورة، لا كشفُ عطب.
    #    ⭐ **والدرس: حارسٌ عُلِّم بعضَ آثارِ الحالة المعذورة يردُّ الحالةَ
    #    المعذورة نفسَها متى ظهرت بأثرٍ لم يُعلَّمه — فالنقصُ في تعليمه.**
    #    ⚖️ والعفوُ يبقى **مشروطاً بالإعلان وحدَه** (‏`drop_surah`)، وكلُّ فاتلٍ
    #    صوتيٍّ حقيقيٍّ يبقى رادّاً ولو مع الإعلان — وهو ما يثبّته
    #    `test_declared_sha_fatal.py` بثمانِ حالاتٍ خمسٌ منها حارسةٌ على الحارس.
    "بصمات الصوت",
)

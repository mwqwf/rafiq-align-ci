...
# ⛔⛔ **قاعدةُ `("ٱ","ا")` أدناه ميتةٌ لا تعمل** — مقيسةٌ 2026-09-07، والعلّةُ ترتيبٌ
#    لا منطق: `_AR` يُطبَّق **قبل** `_FOLD` في `words()`، و`ٱ` = U+0671 **خارج**
#    المدى `ء-ي` (U+0621–U+064A) ⇒ تُحذف قبل أن يراها الطيّ. فـ«ٱللَّهِ» تصير
#    `لله` والتفريغُ يكتبها `الله`. وسائرُ قواعد الطيّ سليمةٌ (مصادرُها داخل المدى).
# ⛔ **ولا يُنقل السطرُ فوق `_AR` بلا إذن المشرف** — الأثرُ قِيس بالأسماء لا بالظنّ:
#    التعرّضُ في **حفصٍ وشعبةَ وحدهما** (16.4% و17.4% من الكلمات؛ وورشٌ وقالون
#    والدوريّ والسوسيّ **صفر**)، وإعادةُ تشغيل `judge()` على 2400 صفٍّ حقيقيٍّ من
#    تقارير `audio_qa` أعطت: `mrifai` 3.75%⇐3.83% · `mukhtar_haj` 2.75%⇐3.50% ·
#    و**شاهدُ الضبط `laghdaf_shinqiti` (ورش) صفرُ انقلاب**. ⇒ **ضجيجٌ لا انحياز،
#    ولم يعبر حكمُ فهرسٍ واحدٍ عتبةَ 5%** — فالطيُّ يجعل 143 مشهوداً يُحكمون
#    بمسطرةٍ غير مسطرة من بعدهم بلا مكسبٍ مقيس.
# ⭐ **وحيث يجب الطيُّ قطعاً — كاشفُ النسبة الخاطئة** (رواية ⇄ رواية): استعمل
#    `normalizer_audit.words_folded`، فالفارقُ المقيس بين نصّي حفصٍ وورشٍ 22.4%
#    من الكلمات و**أربعةَ عشرَ من كلّ خمسةَ عشرَ في الفاتحة أثرُ الرسم لا الرواية**.
#    البرهانُ والجردُ والاختبارُ الذاتيّ في `tools/index_qa/normalizer_audit.py`.
...
def structural(idx, key, allow_unmarked=False, txt_ref=None):
    ...
    info["sha"] = len(sha)
    # ⛔⛔ **المُعفى يُحسب قبل العدّ لا بعده** (2026-09-19، حالةُ `asim`):
    #    الاستثناءُ أدناه أُقرّ 2026-09-12 وطُبّق على فحصَي «الفارغة» و«المكرّرة»
    #    **ولم يُطبَّق على العدّ**، والعدُّ يقع أوّلاً ⇒ فهرسٌ مُعلَنُ الغياب
    #    بسببٍ مقيسٍ يُردّ **قبل أن يُسأل عن إعلانه أصلاً**. والشاهد: `asim`
    #    سورةُ «ص» يردّ المصدرُ عليها 404 وحدَها، فأُعلن غيابُها ثمّ رُدّ بـ
    #    «بصمات الصوت 113/114». ⇒ **استثناءٌ ناقصُ التطبيق يساوي لا استثناء.**
    #    ⭐ والدرسُ الأعمّ: **حارسٌ له استثناءٌ مشروط، وشرطُه يُحسب بعد الحكم،
    #       حارسٌ بلا استثناء** — فترتيبُ الحساب قاعدةٌ غيرُ مكتوبة وهي الأخطر.
    _served = {int(e["ayahId"].split(":")[0]) for e in E}
    # ⛔ الشرطان معاً لا واحد: مُعلَنةٌ **وصفرُ مداخل**. ولا يكفي الإعلانُ وحدَه،
    #    إذ لو أُعفيت سورةٌ **حاضرة** لصار الإعلانُ باباً لشحن صوتِ سورةٍ مكانَ
    #    أخرى — وهو بعينه ما وُضع هذا الحارسُ ليمنعه.
    _exempt = {s for s in declared_drops(idx) if s not in _served}
    # ⛔ والصورتان كلتاهما مشروعتان ولا ثالثةَ لهما: قائمةٌ من 114 فيها خانةٌ
    #    فارغةٌ للمُعفاة (صورةُ `iraoui_warsh`)، أو قائمةٌ قصيرةٌ أُسقطت منها
    #    (صورةُ `asim`). وما عداهما يُردّ — فلا يُشتقّ المسموحُ من عدد المُعلَن
    #    بل من **المُعفى المتحقَّق بالشرطين**.
    if len(sha) not in (114, 114 - len(_exempt)):
    ...
    # ⛔⛔ **الإسقاطُ بالفهرس لا يصحّ إلا على قائمةٍ تامّة** (2026-09-19):
    #    `sha[i]` يقابل السورة `i+1` **ما دامت القائمةُ 114**. فإن كانت قصيرةً
    #    (المُعفاةُ محذوفةٌ منها أصلاً) انزاحت كلُّ بصمةٍ بعدها، فإسقاطُ الخانة
    #    رقم 38 يُسقط **بصمةَ سورةٍ حقيقيّةٍ مخدومة** ويُخرجها من فحص التكرار —
    #    أي يُضعف الحارسَ من حيث أردنا تقويتَه. ⇒ القصيرةُ تُفحص كاملةً، فهي
    #    لا تحوي المُعفاةَ ابتداءً ولا شيءَ فيها يُستثنى.
    _live = ([x for i, x in enumerate(sha) if (i + 1) not in _exempt]
             if len(sha) == 114 else list(sha))
    if _exempt:

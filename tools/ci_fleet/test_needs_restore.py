...
def load(lowcov: float, min_ayahs: int = 3, floor: float = 0.95):
    ns = {"LOWCOV": lowcov, "MIN_AYAHS": min_ayahs, "SMALL_GAP_FLOOR": floor}
    exec(compile(ast.Module([fn], []), "<needs_restore>", "exec"), ns)   # noqa: S102
    return ns["needs_restore"]


def main() -> int:
    # «القديم» = الحالُ قبل إصلاحَي اليوم: قاعٌ 0.75 وبلا قاعٍ نسبيّ إطلاقاً.
    old, new = load(0.75, floor=0.0), load(0.98)
    ...
    cases = [
        ...
        # ⛔ الفاتحةُ — أخطرُ ما كان مهجوراً: نقصُ آيةٍ من سبعٍ سُبعُ السورة.
        (6,   7, "الفاتحةُ تنقصها آيةٌ (28 قارئاً) — سُبعُ السورة", False, True),
        (5,   7, "الفاتحةُ تنقصها آيتان",                          False, True),
        (3,   4, "س112 تنقصها آية — رُبعُ السورة",                  False, True),
        # ✅ ولا يُفتح البابُ للطويل: آيةٌ من 286 نقصٌ يسير، ويبقى خارجاً.
        (285, 286, "البقرةُ تنقصها آيةٌ — 99.7٪ فتبقى خارجاً",      False, False),
        (6234, 6236, "فهرسٌ كاملٌ إلا آيتَين في سورةٍ طويلة", False, False),
    ]
    ok = True
    ...
    # ⛔ حارسُ الإنفاق يبقى على الطويل: نقصُ آيةٍ في سورةٍ طويلةٍ لا عدّاءَ له.
    assert not new(77, 78), "نقصُ آيةٍ في سورةٍ من 78 صار يُنفق عليه عدّاءٌ بلا موجب"
    assert not new(285, 286) and not new(6234, 6236)
    # ✅ وينفتح على القصير المتضرّر وحدَه — وهو المقصودُ من القاع النسبيّ.
    assert new(6, 7) and new(3, 4), "القاعُ النسبيُّ لا يعمل — والفاتحةُ تبقى مهجورة"
    ...
    print("✅ حارسُ الإنفاق يعمل حيث يجب: نقصُ آيةٍ في سورةٍ طويلة"
          " (‏البقرة 285/286) لا عدّاءَ له.")
    print("✅ والقاعُ النسبيُّ ينقذ القصيرَ المتضرّرَ وحدَه: الفاتحةُ 6/7"
          " والإخلاصُ 3/4 دخلتا الطابورَ بعد هجرٍ دائم.")
    print("⛔ وMIN_AYAHS لم يُلغَ — أُضيف إليه قاعٌ نسبيٌّ، فالحدُّ المطلقُ"
          " باقٍ على ما سوى القصير المتضرّر.")

...
> ⛔⛔ **خطوةٌ إلزاميةٌ قبل الإطلاق — نقِّ القائمة من المشهودين:**
>
> ```bash
> python tools/ci_fleet/prune_certified.py <القائمة> --yes
> ```
>
> **سببُها مقيسٌ بثمنه (2026-09-08):** موجةُ `34161827083` أنفقت **شريحةً كاملةً
> من عشرين (5% من سقف الحساب، لساعات)** على إعادة بناء `mrifai` — **وهو مشهودٌ
> مرقّى 20:35Z، قبل إطلاق الموجة بثلاثين دقيقة**. وحارسُ «منجَزٌ سلفاً»
> (`run_shard.sh:109`) **صحيحٌ ولم ينفع**، لأنّ `align.yml:273` يثبّت
> `FORCE_REALIGN: "1"` لكلّ تشغيلة فيرفعه — والمكتوبُ عند المفتاح «يُطلبه
> المشغّلُ عمداً ولا يقع سهواً»، والواقعُ أنه **دائم**.
>
> ⛔ **ولا يُنزع التثبيت:** موجاتُ إعادة البناء كلُّها إعادةُ محاذاةٍ مقصودةٌ
> بوصفةٍ أحدث، وأكثرُ قرّائها لهم بصمةٌ في `timings-staging/` ⇒ نزعُه يُبطلها
> كلَّها (D-178). **فالحارسُ عند التوليد لا عند التشغيل.**
>
> ⛔ **ولا تُنقَّ قائمةُ موجةٍ جارية:** كلُّ شريحةٍ تحسب توزيعَها من الملفّ نفسِه،
> فحذفُ صفٍّ يزيح ما بعده ⇒ إعادةُ تشغيلِ شريحةٍ تعمل على قرّاءٍ آخرين.
>
> ⚠️ و«مشهود» **حالةٌ تتغيّر بين كتابةِ القائمة وإطلاقِها**: ترويسةُ
> `reciters_resume.tsv` تقول «كلُّ غيرِ مشهودٍ له صوت» **وكانت صادقةً حين كُتبت**،
> ثم شُهد `mrifai` فكذبت بلا أن يمسّها أحد. ⇒ يُقاس عند الإطلاق، ولا يُورَث.
...
## ⚖️ حكمٌ على فهارس موجة 2026-09-02 (‏run `33587151546`)

**فهارس هذه الموجة تخرج بلا كتلة `vad` في الترويسة، وتُقبل على أنها
`adaptive-1`** — قرار github-f4، 2026-09-02.

**الحجّة زمنُ الاستنساخ لا زمنُ الرفع:** الشرائح العشرون استنسخت الشجرة عند
إقلاعها **03:28 UTC**، ورقعة `adaptive-2` وكتلة `vad` في `validate.py` وصلتا
`main` **بعد ذلك**. فالفهرس الخارج من هذه الموجة مبنيٌّ بالعتبة القديمة قطعاً،
ولا يحمل شاهداً على نفسه لأن `validate.py` الذي جمّعه لم يكن يكتب الكتلة بعد.

⛔ **وهذا استثناءٌ واحدٌ موقّت لا قاعدة**، وحدّه صريح: **موجة `33587151546`
وحدها**. وكل ما يُنتَج بعدها يجب أن يحمل `vad.versions` من السور — ومن خرج
بلا كتلة بعد هذه الموجة **فعطبٌ لا قِدَم**، لأن كل عدّاءٍ يستنسخ الشجرة عند
إقلاعه فيأخذ الرقعة تلقائياً.

⚠️ **ولا يُعمَّم زمن الاستنساخ حكماً على غير هذه الموجة:** حجّته هنا أنّ
تشغيلةً واحدة معروفة الوقت، مستنسِخةً مرةً واحدة، لا استئناف فيها من كاشٍ
أقدم (بدأت من الصفر بعد الإلغاء). ومتى تخلّف شرطٌ من الثلاثة سقطت الحجّة —
و**الشاهد الوحيد يبقى ما في الملف** (قاعدة github-b9 التي نقضت الحكم بالزمن
في `tareq_qalun`).
